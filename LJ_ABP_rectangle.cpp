...
#define N  6250   // Number of particles
#define LX 250  // x-axis length
#define LY 50  // y-axis length
...
int main(int argc, char *argv[]){
  ...
  if(INITIAL_CONDITION_TYPE){
    ...
    for( int i=0; i<(int)sqrt(N)+1; i++){
      for(int j=0; j<(int)sqrt(N)+1; j++){
        if(i*((int)sqrt(N)+1)+j<N){
          x[i*((int)sqrt(N)+1)+j] = 0.4*LX+(i+0.5)*dcx/5;
    ...
    double dcx = (double)LX/(double)(5*sqrt(N/5)+.0);
    double dcy = (double)LY/(double)(sqrt(N/5)+.0);
    for( int i=0; i<(int)(5*sqrt(N/5)+1); i++){
      for(int j=0; j<(int)(sqrt(N/5)+1); j++){
        if(i*((int)(sqrt(N/5)+1))+j<N){
          x[i*((int)(sqrt(N/5)+1))+j] = (i+0.5)*dcx;
          y[i*((int)(sqrt(N/5)+1))+j] = (j+0.5)*dcy;
        }
      }
    }
  }
